...
! 1. Modify it to include one more parameter sigma_sm, --10/07/2016.
!
! Use f2py -m post_lnprob_module_rsd_HS -h post_lnprob_module_rsd_HS.pyf post_log_prob_rsd_HS.f90 --overwrite-signature to generate
! signature file.
! Use f2py -c --fcompiler=gnu95 post_lnprob_module_rsd_HS.pyf post_log_prob_rsd_HS.f90 to generate the module
!
...
subroutine cal_Pk_model(Pk_linw, Pk_sm, k_t, mu_t, sigma_xy, sigma_z, sigma_sm, sigma_fog, f, b_0, &
                       b_scale, norm_gf, Pk_model, dim_kt)
    implicit none
    integer:: dim_kt, i
    double precision, dimension(dim_kt):: Pk_linw, Pk_sm, k_t, mu_t, Pk_model
    double precision:: sigma_xy, sigma_z, sigma_sm, sigma_fog, f, b_0, b_scale, norm_gf
    double precision:: smooth_term, rsd_fog_term, exp_term
!f2py intent(in):: Pk_linw, Pk_sm, k_t, mu_t, sigma_xy, sigma_z, sigma_sm, sigma_fog, f, b_0, b_scale, norm_gf
!f2py intent(out):: Pk_model
    do i=1, dim_kt
        smooth_term = exp(-(sigma_sm*k_t(i))**2.d0/4.d0)  ! reference from Eq.(5) in Seo et al. 2015
...
subroutine lnprior(theta, params_indices, fix_params, lp, dim_theta, dim_params)
    ...
    double precision:: lp
    double precision:: alpha_1, alpha_2, sigma_xy, sigma_z, sigma_sm, sigma_fog, f, b_0, b_scale
!f2py intent(in):: theta, params_indices, fix_params
!f2py intent(out):: lp
    ...
    sigma_sm = params_array(5) ! smooth scale (reference from Eisenstein et al. 2007).
    sigma_fog = params_array(6)
    f = params_array(7)        ! introduce two more parameters f and sigma_fog in redshift space.
    b_0 = params_array(8)
    ...
    if (alpha_1>-1.d-7 .and. alpha_1<1.1 .and. alpha_2>-1.d-7 .and. alpha_2<1.1 .and. sigma_xy>-1.d-7 .and. sigma_xy<15.0 &
        ...
        .and. sigma_sm>-1.d-7 .and. sigma_sm<30.0 .and. sigma_fog > -1.d-7 .and. sigma_fog < 20.0 .and. f > -1.d-7 &
        .and. f < 4.0) then
        lp = 0.d0
    else
