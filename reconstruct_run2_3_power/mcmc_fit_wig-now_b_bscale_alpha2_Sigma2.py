# Copied the code from mcmc_fit_wig-now_b_bscale_fancy.py, 08/19/2016;
# Modify it to fix \Sigma_xy2=\Sigma_z2, \alpha_xy = \alpha_z.
...
# 5. Replace \Sigma_xy and \Sigma_z by \Sigma_xy2 and \Sigma_z2 parameters, 08/20/2016 -- It doesn't work well actually!!
#    The best fitting could give negative (\Sigma_z)^2!!
#
#!/usr/bin/env python
# -*- coding: utf-8 -*-
...
# alpha_1: alpha perpendicular, alpha_2: alpha parallel, Sigma_xy2, Sigma_z2, b_0, b_scale
...
def lnprior(theta, params_indices, fix_params):
    alpha_1, alpha_2, Sigma_xy2, Sigma_z2, b_0, b_scale = match_params(theta, params_indices, fix_params)
    if -1.e-7<alpha_1<1.1 and -1.e-7<alpha_2<1.1 and -1.e-7<=Sigma_xy2<225.0 and -1.e-7<=Sigma_z2<225.0 and 0.<b_0<6.0 and -100.0<b_scale<100.0:
    #if -1.e-7<alpha_1<1.1 and -1.e-7<alpha_2<1.1 and -1.e-7<Sigma_xy2<100.0 and -1.e-7<Sigma_z2<100.0 and 0.<b_0<6.0 and -1000.0<b_scale<1000.0:
        return 0.0
    return -np.inf
...
def lnlike(theta, params_indices, fix_params, k_p, mu_p, Pk_obs, ivar, tck_Pk_linw, tck_Pk_sm, norm_gf):
    alpha_1, alpha_2, Sigma_xy2, Sigma_z2, b_0, b_scale = match_params(theta, params_indices, fix_params)
    # set alpha_1=alpha_2 and Sigma_xy2 = Sigma_z2
    alpha_1 = alpha_2
    Sigma_xy2 = Sigma_z2
    coeff = 1.0/alpha_1*(1.0+mu_p**2.0*(pow(alpha_1/alpha_2, 2.0)-1.0))**0.5
    k_t = k_p*coeff
    mu_t = mu_p/(alpha_2*coeff)
    k_perp2 = k_t**2.0*(1.0-mu_t**2.0)
    k_para2 = (k_t*mu_t)**2.0
    Pk_linw = interpolate.splev(k_t, tck_Pk_linw, der=0)
    Pk_sm = interpolate.splev(k_t, tck_Pk_sm, der=0)

    Pk_model = (Pk_linw-Pk_sm) * np.exp(-(k_perp2 * Sigma_xy2 + k_para2 * Sigma_z2)/2.0) * (1.0+b_scale * k_t**2.0) * (b_0 * norm_gf)**2.0
...
def write_params(filename, params_mcmc, params_name, reduced_chi2):
    header_line = '# The fitted parameters alpha_1, alpha_2, Sigma_xy2, Sigma_z2, b_0, b_scale (by row) with their upward and downward one sigma error, and the reduced \chi^2.\n'
...
def mcmc_routine(N_params, N_walkers, N_walkersteps, theta, params_T, params_indices, fix_params, k_range, mu_range, Pk_wnow_obs, ivar_Pk_wnow, tck_Pk_linw, tck_Pk_sm, norm_gf):
    ...
    # Set up the sampler.
    # sometimes Sigma_xy2 or Sigma_z2 obtained from optimize routine is negative, we should use abs() passing to the emcee routine.
...
alpha_1, alpha_2, Sigma_xy2, Sigma_z2, b_0, b_scale = 1.0, 1.0, 10.0, 10.0, 1.0, 0.0
all_params = alpha_1, alpha_2, Sigma_xy2, Sigma_z2, b_0, b_scale
all_names = "alpha_1", "alpha_2", "Sigma_xy2", "Sigma_z2", "b_0", "b_scale"
all_temperature = 0.01, 0.01, 0.5, 0.5, 0.1, 1.0

params_indices = [False, True, False, True, True, True] # once you change params_indices, you have to change initial_pvalue as well.
initial_pvalue = np.array([0, 1, 0, 1, 1, 1])
...
odir = './params_{}_wig-now_b_bscale_fitted_mean_alpha2_Sigma2_dset/'.format(rec_dirs[rec_id])
if not os.path.exists(odir):
    os.makedirs(odir)
#------- fit the FoF mean P(k, mu)_wig - P_now by G^2 b^2(P_linw - P_sm)*C_G^2*(1+b_scale*k*k) with isotropic constraints -------#
def fit_fof_mean():
    for z_id in xrange(3):
        ...
        for mcut_id in xrange(N_masscut):
            ...
            for space_id in xrange(1):
                ...
                ofile_params = odir + '{}kave{}.wig-now_b_bscale_mean_fof_a_{}_mcut{}_params{}_isotropic.dat'.format(rec_fprefix[rec_id], sim_space[space_id], sim_a[z_id], mcut_Npar_list[z_id][mcut_id], ''.join(map(str, initial_pvalue)))
                write_params(ofile_params, params_mcmc, params_name, reduced_chi2)


#------- fit Subsample mean P(k, mu)_wig - P_now by G^2 b^2(P_linw - P_sm)*C_G^2*(1+b_scale*k*k) with isotropic constraints -------#
def fit_sub_mean():
    np.random.seed()
    for z_id in xrange(3):
        norm_gf = growth_factor(float(sim_z[z_id]), Omega_m)/G_0
        for space_id in xrange(1):
            ...
            print('Reduced chi2: ', reduced_chi2)
            # output parameters into a file
            ofile_params = odir + '{}kave{}.wig-now_b_bscale_mean_sub_a_{}_params{}_isotropic.dat'.format(rec_fprefix[rec_id], sim_space[space_id], sim_a[z_id], ''.join(map(str, initial_pvalue)))
            write_params(ofile_params, params_mcmc, params_name, reduced_chi2)
