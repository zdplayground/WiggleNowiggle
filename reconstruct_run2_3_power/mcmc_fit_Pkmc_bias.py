#---------------------------------------------------------------------------------------------------
# On the base of mcmc_fit_Pkmc.py, we consider the galaxy bias as a function of k, i.e. b=b1+b2*k, 07/13/2016.
# (It couldn't resolve the \Sigma_xy, \Sigma_z problem, 07/13/2016)
...
# 3. Use P(k, \mu) data files without/with shot noise included.
#
...
# add constA: counting mode-coupling power spectrum term, the prior range is based on the maximum value of shot noise obtained from data files.
    alpha_1, alpha_2, sigma_xy, sigma_z, constA, constB = theta
    if 0.9<alpha_1<1.1 and 0.9<alpha_2<1.1 and 0.0<sigma_xy<15.0 and 0.0<sigma_z<15.0 and constA<0.0 and -2.0<constB<2.0:
        return 0.0
    return -np.inf


# be aware Pk_obs here is the (observed) simulated P_wig/P_now -1.0
def lnlike(theta, k_p, mu_p, Pk_obs, tck_Pk_wovers, tck_Pk_sm, ivar):
    ...
    Pk_fitobs = Pk_fittrue/(1.0 + constA/(Pk_sm*(1.0+constB*k_t)**2.0))
...
def write_params(filename, alpha_1, alpha_2, sigma_xy, sigma_z, constA, constB):
    data_m = np.array([alpha_1, alpha_2, sigma_xy, sigma_z, constA, constB])
    header_line=' The fitted parameters alpha_1, alpha_2, sigma_xy, sigma_z, constA, constB (by row), and their upward and downward one sigma error'
...
def mcmc_routine(k_p, mu_p, Pk_wnow_obs, ivar_Pk_wnow, tck_Pk_wovers, tck_Pk_sm, z_id, space_id):
    ...
    alpha_1, alpha_2, sigma_xy, sigma_z, constA, constB = 0.9, 1.1, 10.0, 10.0, 0.0, 0.0
    
    result = op.minimize(chi2, [alpha_1, alpha_2, sigma_xy, sigma_z, constA, constB], args=(k_p, mu_p, Pk_wnow_obs, tck_Pk_wovers, tck_Pk_sm, ivar_Pk_wnow))
    alpha_1, alpha_2, sigma_xy, sigma_z, constA, constB = result["x"]
    print(alpha_1, alpha_2, sigma_xy, sigma_z, constA, constB)
    
    # Set up the sampler.
    ndim, nwalkers = 6, 48
    # sometimes sigma_xy or sigma_z obtained from optimize routine is negative, we should use abs() passing to the emcee routine.
    pos = [[np.random.uniform(0.9,1.1), np.random.uniform(0.9,1.1), np.random.uniform(0.1, 15.0), np.random.uniform(0.1, 15.0), np.random.uniform(0.0, 1.0), np.random.uniform(0.0, 1.0)] for i in xrange(nwalkers)]
    print(np.shape(pos))
    
    sampler = emcee.EnsembleSampler(nwalkers, ndim, lnprob, a=2.0, args=(k_p, mu_p, Pk_wnow_obs, tck_Pk_wovers, tck_Pk_sm, ivar_Pk_wnow))
    print(type(sampler))
    ...
    alpha_1, alpha_2, sigma_xy, sigma_z, constA, constB = map(lambda v: (v[1], v[2]-v[1], v[1]-v[0]),
...
#          sigma_z={3[0]}+{3[1]}-{3[2]}
#          constA={6[0]}+{6[1]}-{6[2]}""".format(alpha_1, alpha_2, sigma_xy, sigma_z, constA))

    del sampler
    
    return alpha_1, alpha_2, sigma_xy, sigma_z, constA, constB
...
odir = './params_true_mc_bias_fitted_mean_ksorted_mu_masscut/'
    ...
    for mcut_id in xrange(N_masscut):
        ...
        for z_id in xrange(1):
            for space_id in xrange(1): # fit it in real space
                ...
                print(ifile_Pk)
                Pk_wnow_obs = np.loadtxt(ifile_Pk, dtype='f4', comments='#', usecols=(3,)) # be careful that there are k, \mu, P(k, \mu) columns, also P_obs(c2) or P_true(c3)
                ...
                alpha_1, alpha_2, sigma_xy, sigma_z, constA, constB = mcmc_routine(k_p, mu_p, Pk_wnow_obs, ivar_Pk_wnow, tck_Pk_wovers, tck_Pk_sm, z_id, space_id)
                theta = [alpha_1[0], alpha_2[0], sigma_xy[0], sigma_z[0], constA[0], constB[0]]
                print(alpha_1, alpha_2, sigma_xy, sigma_z, constA, constB)
                
                chi2_value = chi2(theta, k_p, mu_p, Pk_wnow_obs, tck_Pk_wovers, tck_Pk_sm, ivar_Pk_wnow)
                print(chi2_value)
                ...
                write_params(ofile_params, alpha_1, alpha_2, sigma_xy, sigma_z, constA, constB)
...
def fit_sub():
    ...
    for z_id in xrange(3):
        for space_id in xrange(2): # fit it in real space
            ...
            alpha_1, alpha_2, sigma_xy, sigma_z, constA, constB = mcmc_routine(k_p, mu_p, Pk_wnow_obs, ivar_Pk_wnow, tck_Pk_wovers, tck_Pk_sm, z_id, space_id)
            #theta = [alpha_1[0], alpha_2[0], sigma_xy[0], sigma_z[0], constA[0]]
            print(alpha_1, alpha_2, sigma_xy, sigma_z, constA, constB)
