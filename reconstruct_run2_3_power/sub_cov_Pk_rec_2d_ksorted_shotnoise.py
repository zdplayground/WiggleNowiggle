# Copy the code cov_Pk_rec_2d_ksorted_shotnoise_masscut.py on 07/06/2016.
# 1. Modify it to get the mean subsample P_wig/P_now of run2 and run3 together running on NERSC
# 2. Get the observed mean subsample P_wig/P_now of run2 and run3 combined data on 07/08/2016
#!/usr/bin/env python
import os
import time
import numpy as np
...
dir0='/project/projectdirs/desi/users/sheejong/DESI_Yu/'
...
#            Cov_Pk = np.dot(Pk_matrix.T, Pk_matrix)/(N_dataset -1.0)
            ...
            ##header_line = ' After sorting k, the mean P(k)_wig/P(k)_now in 2d (k, mu) with shot noise subtracted\n'
            var_name = 'sub_Pk_obs_true_2d_wnw_mean_{}_ksorted_mu/'.format(rec_dirs[rec_id])
            
            filename = "{}kave{}.wnw_mean_a_{}ga.dat".format(rec_fprefix[rec_id], sim_space[space_id], sim_a[z_id])
            ...
            filename = "{}kave{}.now_mean_a_{}ga.dat".format(rec_fprefix[rec_id], sim_space[space_id], sim_a[z_id])
            ...
            filename = "{}kave{}.wig_mean_a_{}ga.dat".format(rec_fprefix[rec_id], sim_space[space_id], sim_a[z_id])
            ...
            ##header_line = ' Cov(P_2d_wnw(k1), P_2d_wnw(k2)), (wnw means wig/now, 2d: k, mu; and k1, k2 are the k bin indices).'
#            var_name = 'sub_Cov_Pk_obs_2d_wnw_mean_{}_ksorted_mu/'.format(rec_dirs[rec_id])
#            header_line = ' Cov(P_obs_2d_wnw(k1), P_obs_2d_wnw(k2)), (obs: observed power spectrum, wnw means wig/now, 2d: k, mu; and k1, k2 are the k bin indices).'
#            write_output(odir_prefix, var_name, header_line, rec_id, space_id, z_id, Cov_Pk)
    t1 = time.clock()
