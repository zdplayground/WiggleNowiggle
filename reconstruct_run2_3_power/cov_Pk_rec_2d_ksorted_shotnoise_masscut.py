# made on 07/05/2016
# Follow the basic routine as run_2_3_cov_Pk_2d_ksorted_shotnoise_masscut.py to calculate the mean P_wig/P_now and its covariance matrix.
# Modify it to get the mean P_wig/P_now of run2 and run3 together running on NERSC, 07/05/2016
#
#!/usr/bin/env python
import os
import time
            ...
            for space_id in xrange(2):
                ...
                for run_id in xrange(2):
                    for sim_seed_id in xrange(10):
                        k_i, Pk_now_obs, Pk_now_true = read_fftPk_file(dir0, z_id, mcut_id, rec_id, space_id, sim_wig[run_id][0], sim_seed_id)
                        Pk_mnow_obs = np.vstack([Pk_mnow_obs, Pk_now_obs])
                        Pk_mnow_true = np.vstack([Pk_mnow_true, Pk_now_true])
                        
                        k_i, Pk_wig_obs, Pk_wig_true = read_fftPk_file(dir0, z_id, mcut_id, rec_id, space_id, sim_wig[run_id][1], sim_seed_id)
...
#                Cov_Pk = np.dot(Pk_mwnw_obs.T, Pk_mwnw_obs)/(N_dataset -1.0)
                ...
                var_name = 'fof_Pk_obs_true_2d_wnw_mean_{}_ksorted_mu_masscut/'.format(rec_dirs[rec_id])
