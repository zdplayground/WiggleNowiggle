# Copied the code from mcmc_fit_fofmasscut.py, 07/13/2016;
...
def mcmc_routine(k_range, mu_range, Pk_wnow_obs, ivar_Pk_wnow, ofile_params, name_id, space_id):
    ...
    sampler = emcee.EnsembleSampler(nwalkers, ndim, lnprob, a=2.0, args=(k_range, mu_range, Pk_wnow_obs, tck_Pk_true, ivar_Pk_wnow))
    print(type(sampler))
...
def fit_fof_mean():
    ...
    for rec_id in xrange(1, 2):
        odir = './params_{}_wnw_diff_fitted_mean_dset/'.format(rec_dirs[rec_id])
        ...
        for mcut_id in xrange(N_masscut):
            ...
            for z_id in xrange(1, 3):
                for space_id in xrange(1):
                    ...
                    ofile_params = odir + '{}kave{}.wnw_diff_mean_fof_a_{}_mcut{}_params.dat'.format(rec_fprefix[rec_id], sim_space[space_id], sim_a[z_id], mcut_Npar_list[z_id][mcut_id])
                    alpha_1, alpha_2, sigma_xy, sigma_z = mcmc_routine(k_p, mu_p, Pk_wnw_diff_obs, ivar_Pk_wnow, ofile_params, name_id, space_id)
                    chi_square = chi2([alpha_1, alpha_2, sigma_xy, sigma_z], k_p, mu_p, Pk_wnw_diff_obs, tck_Pk_true, ivar_Pk_wnow)
                    print(chi_square)
        ...
        for mcut_id in xrange(N_masscut):
            ...
            for z_id in xrange(3):
                for space_id in xrange(1):
                    ...
                    ofile_params = odir + '{}kave{}.wnw_diff_mean_sub_a_{}_params.dat'.format(rec_fprefix[rec_id], sim_space[space_id], sim_a[z_id])
                    alpha_1, alpha_2, sigma_xy, sigma_z = mcmc_routine(k_p, mu_p, Pk_wnw_diff_true, ivar_Pk_wnow, ofile_params, name_id, space_id)
                    chi_square = chi2([alpha_1, alpha_2, sigma_xy, sigma_z], k_p, mu_p, Pk_wnw_diff_true, tck_Pk_true, ivar_Pk_wnow)
                    print(chi_square)
                    print('Reduced chi2: ', chi_square/(N_fitbin-4), '\n')
