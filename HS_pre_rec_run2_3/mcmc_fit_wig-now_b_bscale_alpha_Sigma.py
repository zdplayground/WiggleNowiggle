...
# Modify it to fix \Sigma_xy=\Sigma_z, \alpha_xy = \alpha_z.
...
def lnprior(theta, params_indices, fix_params):
    ...
    if -1.e-7<alpha_1<1.1 and -1.e-7<alpha_2<1.1 and -1.e-7<=sigma_xy<15.0 and -1.e-7<=sigma_z<15.0 and 0.<b_0<6.0 and -100.0<b_scale<100.0:
...
def mcmc_routine(N_params, N_walkers, N_walkersteps, theta, params_T, params_indices, fix_params, k_range, mu_range, Pk_wnow_obs, ivar_Pk_wnow, tck_Pk_linw, tck_Pk_sm, norm_gf):
    ...
    # guarantee \Sigma is positive.
    theta_optimize[1] = abs(theta_optimize[1])
...
if not os.path.exists(odir):
    os.makedirs(odir)
#------- fit the FoF mean P(k, mu)_wig - P_now by G^2 b^2(P_linw - P_sm)*C_G^2 with isotropic constraints -------#
def fit_fof_mean():
...
#------- fit Subsample mean P(k, mu)_wig - P_now by G^2 b^2(P_linw - P_sm)*C_G^2 with isotropic constraints -------#
def fit_sub_mean():
    np.random.seed()
    for z_id in xrange(3):
        norm_gf = growth_factor(float(sim_z[z_id]), Omega_m)/G_0
