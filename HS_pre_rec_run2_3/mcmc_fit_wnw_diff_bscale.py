# Copied the same code from the folder reconstruct_run2_3_power, 08/08/2016;
...
# 3. From MCMC results, the error of fitted \alpha is about 0.0001 assigning 50 walker
# each with 4000 steps. And the error of \Sigma is about 0.01.
# 4. On the base of the model in mcmc_fit_wnw_diff.py, we add the term (1+b_scale * k^2).
#    -- The fitting result is bad, which is reasonable because (1+b_scale * k^2) has the same or opposite function from the exponential term.
#
#!/usr/bin/env python
# -*- coding: utf-8 -*-
...
# alpha_1: alpha perpendicular, alpha_2: alpha parallel, mu_p: mu prime, two sigma components: sigma_xy, sigma_z
    alpha_1, alpha_2, sigma_xy, sigma_z, b_0, b_scale = theta
    if 0.9<alpha_1<1.1 and 0.9<alpha_2<1.1 and 0.0<=sigma_xy<15.0 and 0.0<=sigma_z<15.0 and 1.0<=b_0<10.0 and -1000.0<b_scale<1000.0:
        return 0.0
    return -np.inf
...
def lnlike(theta, k_p, mu_p, Pk_obs, tck_Pk_true, ivar):
    ...
    Pk_model = (Pk_t-1.0)*np.exp(k_t*k_t *(mu_t*mu_t*(sigma_xy+sigma_z)*(sigma_xy-sigma_z)-sigma_xy**2.0)/2.0) * (1.0+b_scale * k_t**2.0)* b_0**2.0 *
...
dir0='./run2_3_fof_Pk_2d_wnw_diff_mean_DD_ksorted_mu_masscut/'

inputf = dir0 +'kaver.fof_wnw_diff_mean_a_0.5000_mcut34.dat'
        ...
        for mcut_id in xrange(N_masscut):
            ...
            for z_id in xrange(3):
                for space_id in xrange(1):
                    ifile_Pk = './{}_fof_Pk_2d_wnw_diff_mean_{}_ksorted_mu_masscut/{}kave{}.fof_wnw_diff_mean_a_{}_mcut{}.dat'.format(sim_run, rec_dirs[rec_id], rec_fprefix[rec_id], sim_space[space_id], sim_a[z_id], mcut_Npar_list[z_id][mcut_id])
                    Pk_wnw_diff_obs = np.loadtxt(ifile_Pk, dtype='f4', comments='#', usecols=(2,)) # be careful that there are k, \mu, P(k, \mu) columns.
                    
                    ifile_Cov_Pk = './{}_Cov_fof_Pk_2d_wnw_diff_mean_{}_ksorted_mu_masscut/{}kave{}.fof_wnw_diff_mean_a_{}_mcut{}.dat'.format(sim_run, rec_dirs[rec_id], rec_fprefix[rec_id], sim_space[space_id], sim_a[z_id], mcut_Npar_list[z_id][mcut_id])
...
def fit_sub_mean():
    ...
    for rec_id in xrange(1):
        ...
        for z_id in xrange(3):
            for space_id in xrange(1):
                ifile_Pk = './{}_sub_Pk_2d_wnw_diff_mean_{}_ksorted_mu/{}kave{}.sub_wnw_diff_mean_a_{}.dat'.format(sim_run, rec_dirs[rec_id], rec_fprefix[rec_id], sim_space[space_id], sim_a[z_id])
                Pk_wnw_diff_true = np.loadtxt(ifile_Pk, dtype='f8', comments='#', usecols=(2,)) # be careful that there are k, \mu, P(k, \mu) columns.
                
                ifile_Cov_Pk = './{}_Cov_sub_Pk_2d_wnw_diff_mean_{}_ksorted_mu/{}kave{}.sub_wnw_diff_mean_a_{}.dat'.format(sim_run, rec_dirs[rec_id], rec_fprefix[rec_id], sim_space[space_id], sim_a[z_id])
