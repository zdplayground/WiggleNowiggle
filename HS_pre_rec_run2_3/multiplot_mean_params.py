#! ~/vert/bin/python
# Copied the code multiplot_mean_individual.py from the folder ../reconstruct_run2_3_power, renamed it to multiplot_mean_params.py. 07/16/2016
...
def plot_alpha_fofsub_bias(bias_array, y1, y2, bias_0, sub_y1, sub_y2, N_masscut, space_id, sim_space, title_space, odir, figname):
    ...
    plt.ylabel(r'$\alpha-1.0$ (%)', fontsize=16)
...
def plot_Sigma_fofsub_bias(bias_array, y1, y2, bias_0, sub_y1, sub_y2, N_masscut, space_id, sim_space, title_space, sim_a, omega_m0, omega_l, odir, figname):
    ...
    plt.xlabel('bias', fontsize=16)
    plt.ylabel(r'$\Sigma$ $[Mpc/h]$', fontsize=16)
...
def plot_P_kmu(k, P_kmu, odir, space_id, z_id, mcut_id, filename):
    ...
    plt.ylabel(r"$(P_{wig}/P_{now})_{\mathrm{mean}}$")
...
def fit_obs(theta, k_p, mu_p, tck_Pk_true):
...
def overplot_fit_obs(k_p, k_smooth, Pk_obs, Pk_fit, Pk_wovers, var_Pk_wnw_mean, space_id, z_id, reduced_chi2, odir, fname, mu_boundary=None):
    plt.figure(figsize=(13, 8))
    plt.errorbar(k_p, Pk_obs, yerr=var_Pk_wnw_mean**0.5, fmt='.', lw=1.5, label='obs')
    plt.plot(k_p, Pk_fit, 'r-', lw=1.5, label='fit')
    plt.plot(k_smooth, Pk_wovers-1.0, '-', label='theory')
    plt.xscale("log")
    ...
    plt.ylim([-0.1, 0.1])
    ##plt.title(r"z={0} $\Delta\chi^2={1:.2f}$ {2}".format(sim_z[z_id], reduced_chi2, title_space[space_id]), fontsize=20)
    plt.title(r"$z={0}$, ${1}<\mu<{2:.1f}$ $\Delta\chi^2={3:.2f}$ {4}".format(sim_z[z_id], mu_boundary[0], mu_boundary[1], reduced_chi2, title_space[space_id]), fontsize=24)
...
#    fof_idir = './{}_params_fitted_mean_dset_fofPkobs_subPktrue/'.format(rec_dirs[rec_id])
#    sub_idir = fof_idir
#    odir = './{}_figs_fofobs_subtrue/'.format(rec_dirs[rec_id])
    fof_idir = './{}_params_fitted_mean_dset_fofPktrue/'.format(rec_dirs[rec_id])
    sub_idir = './{}_params_fitted_mean_dset_fofPkobs_subPktrue/'.format(rec_dirs[rec_id])
    odir = './{}_figs_foftrue_subtrue/'.format(rec_dirs[rec_id])
...
##        figname_Sigma = '{}kave{}_wnw_fofobs_subtrue_mean_Sigma_bias_mu_{}_fofmasscut_sub.pdf'.format(rec_fprefix[rec_id], sim_space[space_id], N_mu)
        figname_alpha = '{}kave{}_wnw_foftrue_subtrue_mean_alpha_bias_mu_{}_fofmasscut_sub.pdf'.format(rec_fprefix[rec_id], sim_space[space_id], N_mu)
        figname_Sigma = '{}kave{}_wnw_foftrue_subtrue_mean_Sigma_bias_mu_{}_fofmasscut_sub.pdf'.format(rec_fprefix[rec_id], sim_space[space_id], N_mu)
...
def show_fof_P_kmu():
    fof_idir = './{}_fof_Pk_obs_true_2d_wnw_mean_{}_ksorted_mu_masscut/'.format(sim_run, rec_dirs[rec_id])
...
#    params_idir = './{}_{}_params_fitted_mean_dset/'.format(sim_run, rec_dirs[rec_id])
    params_idir = './{}_params_fitted_mean_dset_fofPkobs_subPktrue/'.format(rec_dirs[rec_id])
#    Cov_Pk_idir = './{}_Cov_Pk_2d_wnw_mean_{}_ksorted_mu_masscut/'.format(sim_run, rec_dirs[rec_id])
    Cov_Pk_idir = './{}_Cov_fof_Pk_obs_2d_wnw_mean_{}_ksorted_mu_masscut/'.format(sim_run, rec_dirs[rec_id])
    ...
    for space_id in xrange(1):
        for z_id in xrange(1):
            for mcut_id in xrange(N_masscut[z_id]):
                ...
                print(fof_ifile)
                k, mu, P_kmu_obs = np.loadtxt(fof_ifile, dtype='f8', comments='#', usecols=(0,1,2), unpack=True)
                ...
                P_kmu_fit = fit_obs(aass, k_p, mu_p, tck_Pk_true)
...
def show_Subsample_P_kmu():
    ...
    for space_id in xrange(1):
        for z_id in xrange(3):
            ...
            overplot_fit_obs(k_p, P_kmu_obs, P_kmu_fit, var_Pk_wnw_mean, space_id, z_id, reduced_chi2, odir, figname, mu_boundary)
