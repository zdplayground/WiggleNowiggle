# Copied the same code from the folder reconstruct_run2_3_power, 07/15/2016
#
#!/usr/bin/env python
# -*- coding: utf-8 -*-
...
# MCMC routine
def mcmc_routine(k_range, mu_range, Pk_wnw_obs, ivar_Pk_wnow, ofile_params, name_id, space_id):
...
dir0='./run2_3_fof_Pk_obs_true_2d_wnw_mean_DD_ksorted_mu_masscut/'

inputf = dir0 +'kaver.wnw_mean_fof_a_0.5000_mcut34.dat'
...
def fit_mean():
    ...
    odir = './{}_params_fitted_mean_dset_fofPktrue/'.format(rec_dirs[rec_id])   # correspondingly using the 4th column P_true in the P(k, mu) data file
    ...
    for mcut_id in xrange(N_masscut):
        ...
        for z_id in xrange(3):
            for space_id in xrange(1):
                ...
                ##Pk_wnw_obs = np.loadtxt(ifile_Pk, dtype='f4', comments='#', usecols=(2,)) # be careful that there are k, \mu, P(k, \mu)_obs and P_true columns.
                Pk_wnw_obs = np.loadtxt(ifile_Pk, dtype='f4', comments='#', usecols=(3,))
                ifile_Cov_Pk = './{}_Cov_fof_Pk_true_2d_wnw_mean_{}_ksorted_mu_masscut/{}kave{}.wnw_mean_fof_a_{}_mcut{}.dat'.format(sim_run, rec_dirs[rec_id], rec_fprefix[rec_id], sim_space[space_id], sim_a[z_id], mcut_Npar_list[z_id][mcut_id])
                ...
                alpha_1, alpha_2, sigma_xy, sigma_z = mcmc_routine(k_p, mu_p, Pk_wnw_obs, ivar_Pk_wnw, ofile_params, name_id, space_id)
