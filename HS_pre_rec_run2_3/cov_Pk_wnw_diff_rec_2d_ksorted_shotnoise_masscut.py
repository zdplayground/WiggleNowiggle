# Copy the same code from the folder reconstruct_run2_3_power, 08/09/2016.
# 1. Get the mean (Pobsw-Pobsnw)/(G^2b^2*P_linnw) and its covariance matrix.
# 2. Get the mean (Pobsw-Pobsnw) and its covariance matrix. -- 08/04/2016
#
#!/usr/bin/env python
import os
import time
...
####--------------------------------------------------------------------------------------------------------------####
####----------------------------- calculate the mean (P_wig-P_now)/(b^2 G^2 Psm) and its covariance matrix -----------------------####
#-- This is form MCMC fitting without the bias parameter b, but with the parameter b_scale.
    ...
    for z_id in xrange(3):
        for mcut_id in xrange(N_masscut):
            for space_id in xrange(1):
                ...
                var_name = 'Cov_Pk_obs_2d_wnw_mean_{}_ksorted_mu_masscut/'.format(rec_dirs[rec_id])
                filename = "{}kave{}.wig_minus_now_mean_fof_a_{}_mcut{}.dat".format(rec_fprefix[rec_id], sim_space[space_id], sim_a[z_id], mcut_Npar_list[z_id][mcut_id])
                header_line = ' Cov(P_obs_2d_wnw_diff(k1), P_obs_2d_wnw_diff(k2)), (obs: including shot noise; wnw_diff means Pwig-Pnow, 2d: k, mu; and k1, k2 are the k bin indices).'
                write_output(odir_prefix, var_name, header_line, rec_id, space_id, z_id, mcut_id, Cov_Pk, filename)
