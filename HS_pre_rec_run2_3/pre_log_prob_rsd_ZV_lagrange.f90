! Copy the code from pre_log_prob_rsd_ZV.f90, modify it to fit the new equation in redshift space for pre-reconstruction.
! (Pwig-Pnow)_obs = (b1^2+2*f*b1*nu^2+f^2*nu^4+(b1*bp+f*bp*nu^2)*(k/k_L)^2)*(Pwig-Pnow)_linear*exp(-k^2*(1+f*(2+f)*nu^2)*Sigma^2)
! The likelihood function is referenced from Zvonimir Vlah. -- 11/10/2016.
! Use f2py -m pre_lnprob_module_rsd_ZV_lagrange -h pre_lnprob_module_rsd_ZV_lagrange.pyf pre_log_prob_rsd_ZV_lagrange.f90 --overwrite-signature to generate
! signature file.
! Use f2py -c --fcompiler=gnu95 pre_lnprob_module_rsd_ZV_lagrange.pyf pre_log_prob_rsd_ZV_lagrange.f90 to generate the module
!
...
!f2py intent(out):: Pk_model
    do i=1, dim_kt
        ...
        t2 = exp(-(k*sigma)**2.d0 * (1.d0+f*(2.d0 + f)*mu*mu))
...
!f2py intent(out):: lp
    ...
    if (alpha_1>-1.d-7 .and. alpha_1<1.1 .and. alpha_2>-1.d-7 .and. alpha_2<1.1 .and. sigma>-1.d-7 .and. sigma<100.0 &
        .and. f > 0.0 .and. f < 4.0 .and. b_0>0. .and. b_0<6.0 .and. b_scale>-1000.0 .and. b_scale<1000.0) then
        lp = 0.d0
    else
